...
namespace Map
{
    public partial class PivotMainPage : PhoneApplicationPage
    {
        ...
        void UpdateSongTitle()
        {
            ...
            {
                string s = "▶ " + MediaPlayer.Queue.ActiveSong.Name + " ¤ " + MediaPlayer.Queue.ActiveSong.Artist.Name;
                tbSongTitle.Text = ShortenString(s);
                myStoryboard.Begin();
                tbSongTitle.Foreground = myAnimatedBrush;
            }
            else
            {
                tbSongTitle.Text = "";
        ...
        void ApplySettings()
        {
            ...
            var user = db.UserDatas.FirstOrDefault();
        ...
        void OnBackKeyPressed(object sender, CancelEventArgs e)
        {
            ...
            if (!longListMultiSelector.IsSelectionEnabled)
            {
            ...
            {
                longListMultiSelector.IsSelectionEnabled = false;
                btSelect_Delete_Appearance("Select");
                e.Cancel = true;
            }
        }
        ...
        private void btStart_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                if (((AppSettings)IsolatedStorageSettings.ApplicationSettings["AppSettings"])._AutoStartMusic && MediaPlayer.State != MediaState.Playing)
                {
        ...
        private string getTotalDurationString(IEnumerable<RunData> data)
        {
            int result = 0;
            if (data != null)
        ...
        async private void btShowLocation_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                mylocator.MovementThreshold = 1;
                Geoposition myLocation;

                myLocation = await mylocator.GetGeopositionAsync();
                prgBar.Visibility = Visibility.Collapsed;
                tbFindingLocation.Visibility = Visibility.Collapsed;
                prgBar.IsIndeterminate = false;
        ...
        private void BuildLocalizedApplicationBar()
        {
            ...
            mnuitem.Text = AppResources.MusicPlayer;
            ...
            mnuitem = (ApplicationBarMenuItem)ApplicationBar.MenuItems[5];
            mnuitem.Text = AppResources.ShareThisAppSMS;

            mnuitem = (ApplicationBarMenuItem)ApplicationBar.MenuItems[6];
            mnuitem.Text = AppResources.ShareThisAppEmail;
        ...
        void btSelect_Click(object sender, EventArgs e)
        {
            try
            {
                if (btSelect_AB.Text == AppResources.Select)
                {
                    ...
                    longListMultiSelector.IsSelectionEnabled = true;
                }
                else
                {
                    if (longListMultiSelector.SelectedItems.Count != 0)
                        if (MessageBox.Show(AppResources.DeleteConfirmMsg, AppResources.Attention, MessageBoxButton.OKCancel) == MessageBoxResult.OK)
                        {
                            ...
                            foreach (RunData item in selectedItems)
                                data.DeleteOnSubmit(data.First(p => p.No == item.No));
                            db.SubmitChanges();
        ...
        private List<Group<RunData>> GetRunGroups()
        {
            IEnumerable<RunData> runList = GetRunDataList();
            return GetItemGroups(runList, c => c.Datetime);
        }

        private static IEnumerable<RunData> GetRunDataList()
        {
            JustRunDataContext db = new JustRunDataContext(JustRunDataContext.ConnectionString);
            return db.RunDatas;
        }
        ...
        private static List<Group<T>> GetItemGroups<T>(IEnumerable<T> itemList, Func<T, DateTime> getKeyFunc)
        {
            IEnumerable<Group<T>> groupList = from item in itemList
                                              ...
                                              select new Group<T>(g.Key.ToShortDateString(), g);
        ...
        private void Grid_Tap(object sender, System.Windows.Input.GestureEventArgs e)
        {
            try
            {
                ...
                var data = db.RunDatas;
                var item = data.Single(p => p.No == int.Parse(tb.Text));
        ...
        private void tbSongTitle_Tap(object sender, System.Windows.Input.GestureEventArgs e)
        {
            NavigationService.Navigate(new Uri("/MusicPage.xaml", UriKind.Relative));
        }

        private void abmnMusicPlayer_Click(object sender, EventArgs e)
        {
            NavigationService.Navigate(new Uri("/MusicPage.xaml", UriKind.Relative));
        }

        private void abmnShareSMS_Click(object sender, EventArgs e)
        {
            SmsComposeTask smsTask = new SmsComposeTask();
            smsTask.Body = AppResources.ShareMsg; // the string containing the sms body
            smsTask.Show(); // this will invoke the native sms edtior
        }

        private void abmnShareEmail_Click(object sender, EventArgs e)
        {
            EmailComposeTask emailComposeTask = new EmailComposeTask();

            emailComposeTask.Subject = AppResources.ShareSubject;
            emailComposeTask.Body = AppResources.ShareMsg;
